...
addEventHandler( "onClientResourceStart", resourceRoot,
	function()
		...
		local set1 = gettok(settings, 1, "|")
		if not(set1) then 
			fileWrite(file, "|0|0|0|")
			fileFlush(file)
		else
			if(tonumber(set1) == 1) then
				triggerEvent( "switchRoadshine3", resourceRoot, true )
			end
		end
		fileClose(file)
...
function ShaderInfoList.getShaderInfoForStrength(strength)
	...
	-- Create a new shader
	local shader = dxCreateShader ( "shaders/roadshine/roadshine3.fx", 0, maxEffectDistance )
	if not shader then
