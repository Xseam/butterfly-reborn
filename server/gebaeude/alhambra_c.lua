local Marker = createMarker(499.74032592773, -20.493223190308, 1000.6796875, "corona", 1.0, 255, 255, 255, 255) setElementInterior(Marker, 17)
-- Ped --
...
function()
	...
	setPedAnimation( Keeper )
	setPedRotation(Keeper, 90)
	setElementPosition(Keeper, 502.50198364258, -23.778837203979, 1000.6796875)
	end
end, 4000, 0)

addEventHandler("onClientPedDamage", getRootElement(),
function()
	if(source == Keeper) then
		cancelEvent()
...
function createSaufenMenue()
	...
	Saufenlabel[1] = guiCreateLabel(10,27,256,19,"Willkommen im Alhambra! Was darf's sein?",false,Saufenfenster[1])
...
function(hitElement)
if not (hitElement == gMe) then return end
	createSaufenMenue()
end)